...
class CodebaseMapping(BaseModel):
        ...
        for child in ast.walk(node):
            if isinstance(child, ast.Name) and isinstance(child.ctx, ast.Load):
                for alias, key in self.file_aliases.items():
                    if (
                        child.id in self.mapping
                        and self.mapping[child.id]["file_alias"] == key
                    ):
                        imports.append(f"{child.id}@{key}")
                        self.mapping[f"{child.id}@{key}"]["imported_by"].append(
                            f"{node.name}@{self.file_aliases[alias]}"
                        )
        return imports
    ...
    def update_mapping(self, diff: str) -> None:
        ...
        for patched_file in patch_set:
            ...
            if file_path not in self.mapping.file_aliases:
                self.mapping.file_aliases[file_path] = str(
                    len(self.mapping.file_aliases) + 1
                )
                ...
                for line in hunk.source_lines():
                    if line.startswith("def "):
                        ...
                        if file_path in self.mapping.mapping:
                            if (
                                function_name
                                in self.mapping.mapping[file_path].functions
                            ):
                                del self.mapping.mapping[file_path].functions[
                                    function_name
                                ]
    ...
    def _apply_patch_to_file(self, file_path: str, patched_file) -> str:
        ...
        for hunk in patched_file:
            for line in hunk:
                if line.is_added:
                    lines.insert(
                        hunk.target_start + line.target_line_no - 1, line.value
                    )
        ...
        for capture in captures:
            ...
            if capture_type == "module_name":
                ...
                if current_function and current_function in node.functions:
                    ...
                    node.functions[current_function].summary = self.generate_summary(
                        function_body
                    )
                ...
                for imported_function in imported_functions:
                    if module in self.mapping.mapping:
                        ...
                        if imported_function in imported_node.functions:
                            imported_node.functions[
                                imported_function
                            ].imported_by.append(
                                f"{function_name}@{self.mapping.file_aliases[file_path]}"
                            )
    ...
    def save_mapping(self, output_file: str) -> None:
        ...
        with open(output_file, "w") as f:
            json.dump(
                {"mapping": self.mapping, "file_aliases": self.file_aliases},
                f,
                indent=2,
            )
...
def main():
    ...
    parser.add_argument(
        "action",
        choices=["build", "update", "search", "save", "load"],
    ...
    codebase_mapping = CodebaseMapping()
