 ...
 * @returns The summary.
 */
export const generateTestResultsSummary = (testResults: { [key: string]: testResult }): string => {
  ...
  const counts = Object.values(testResults).reduce(
    ...
    Object.fromEntries(Object.values(testResult).map(result => [result, 0])) as Record<
      testResult,
      number
    >
  )
