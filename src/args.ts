...
export const getYargs = async (): Promise<ReviewArgs> => {
  const argv = yargs(process.argv.slice(2))
    ...
    .option("summary", {
      description: "Enables emoji summary in the review.",
      type: "boolean",
      default: false
    })
  ...
  if (argv.shouldCommentPerFile && !argv.isCi) {
    throw new Error("The 'commentPerFile' flag requires the 'ci' flag to be set.")
  }

  if (argv.isCi === PlatformOptions.GITLAB && argv.shouldCommentPerFile) {
    logger.warn("The 'commentPerFile' flag only works for GitHub, not for GitLab and AzureDevOps.")
  }
