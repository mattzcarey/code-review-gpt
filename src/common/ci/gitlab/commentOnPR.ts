...
export const commentOnPR = async (comment: string, signOff: string): Promise<void> => {
  try {
    const { gitlabToken, projectId, mergeRequestIIdString, gitlabHost } = getGitLabEnvVariables()
    ...
    if (!projectId) {
      throw new Error("GitLab project ID not provided")
    }

    if (!mergeRequestIIdString) {
      throw new Error("GitLab merge request IID not provided")
    }
    ...
    if (botComment) {
      await api.MergeRequestNotes.edit(projectId, mergeRequestIId, botComment.id, {
        body: botCommentBody
      })
    } else {
